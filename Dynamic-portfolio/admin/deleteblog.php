...
if (isset($_SESSION['user'])) 
{
    ...
    $res3=unlink($file);
    if($res3>=1){
    echo "<script>alert('Blog Page deleted successfully from server')</script>";
    }
    else{
        echo"<script>alert('Unable to delete Blog Page from server')</script>";
    }

    $res=mysqli_query($con,"delete from blog where `id`='".$id."'");
